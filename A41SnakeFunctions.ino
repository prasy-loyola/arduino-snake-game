...
void drawSnake() {
  unsigned char columnValues[8] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  for (int i = 0; (i < 64) && (snake_body[i] > 0x00); i++) {
    unsigned char addr = getAddr(snake_body[i]) ;
    columnValues[addr] = columnValues[addr] | getValue(snake_body[i]);
  }
  unsigned char current_food_addr = getAddr(food_position);
...
void moveSnake(int direction) {
  ...
  unsigned char new_head = snake_body[0]  + direction;
  ...
  if (grow) {
    unsigned char temp = snake_body[0];
    for (int i = 1; i < 63; i++) {
      unsigned char temp2 = snake_body[i];
      snake_body[i] = temp;
      temp = temp2;
    }
